...
// Mục đích tạo Promise này để khi nào gọi api refresh_token xong xuôi
  ...
  if (error.response?.status === 410 && !originalRequests._retry) {
    ...
    if (!refreshTokenPromise) {
      refreshTokenPromise = refreshTokenAPI()
        .then(data => {
          return data?.accessToken // đồng thời accessToken đã nằm trong httpOnly cookie (xử lý từ phía BE)
        })
        .catch(() => {
          axiosReduxStore.dispatch(logoutUserAPI(false))// Nếu nhận bất kỳ lỗi nào từ api refresh token thì cứ logout luôn
        })
        .finally(() => {
